...
from recurrl_jax.agents.base_agent import ActorCriticModel,BaseAgent
...
class A2CAgent(BaseAgent):
    ...
    def __init__(self,train_envs,eval_env,rollout_len,
                        ...
                        entropy_coef=0.01, value_loss_coef=0.5,arg_max=False,
        ...
        def loss_fn(params,random_key,observations,actions,rewards,terminations,h_tickminus1):
            ...
            #calculate the values and log likelihoods for timesteps {tick} - {tick+rollout_len+1}
            logits_diff,values_diff,_=self.actor_critic_fn(random_key,params,observations,terminations,h_tickminus1)
            #calculate lambda returns G_{tick} - G_{tick+rollout_len}
    ...
    def step(self,random_key):
        ...
        h_tickminus1=jax.tree.map(lambda x: x,self.h_tickminus1) #copy the hidden state
